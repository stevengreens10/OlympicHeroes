...
/**
 * A task to handle ongoing potion effects
 * @author Steven Green
 *
 */
public class EffectsTask implements Runnable {
    ...
    public void run() {
        ...
        for (Player p : Bukkit.getServer().getOnlinePlayers()) {
            ...
            if (mainItem != null && (mainItem.getType() == Material.DIAMOND_SWORD
                    || mainItem.getType() == Material.GOLD_SWORD || mainItem.getType() == Material.IRON_SWORD
                    || mainItem.getType() == Material.STONE_SWORD || mainItem.getType() == Material.WOOD_SWORD)) {
                swordInMain = true;
            }

            if (mainItem != null && (mainItem.getType() == Material.DIAMOND_AXE
                    || mainItem.getType() == Material.GOLD_AXE || mainItem.getType() == Material.IRON_AXE
                    || mainItem.getType() == Material.STONE_AXE || mainItem.getType() == Material.WOOD_AXE)) {
                axeInMain = true;
            }

            if (mainItem != null && (mainItem.getType() == Material.DIAMOND_PICKAXE
                    || mainItem.getType() == Material.GOLD_PICKAXE || mainItem.getType() == Material.IRON_PICKAXE
                    || mainItem.getType() == Material.STONE_PICKAXE || mainItem.getType() == Material.WOOD_PICKAXE)) {
                pickInMain = true;
            }
            ...
            boolean hasShield = (offItem != null) ? OHItems.isItemSimilarTo(offItem, OHItems.aegisShield, true) : false;

            if (ohPlayer.getLevel("Athena") >= 5) {
                if (!hasShield && swordInMain) {
                    if (offItem == null || offItem.getType() == Material.AIR) {
                        p.getInventory().setItemInOffHand(OHItems.aegisShield);
                    }
                }
            }
    ...
    /**
     * Determines whether an effect should be applied to a player.
     * If the effect they currently have is higher than the new effect, the new
     * effect should not be applied
     * @param player The player to apply the effect to
     * @param type The type of the effect
     * @param level The level of the new effect
     * @return True / false whether the new effect should be applied or not
     */
