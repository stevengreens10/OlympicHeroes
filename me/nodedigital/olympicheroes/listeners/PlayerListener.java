...
public class PlayerListener implements Listener {
    ...
    public void onInvClick(InventoryClickEvent e) {
        ...
        for (ItemStack it : OHItems.disallowedItems) {
        ...
        if (player.getKiller() instanceof Player) {
            ...
            for (String god : Variables.GODS) {
                if (!Cooldowns.killCooldownMap.containsKey(killer)
                        || !Cooldowns.killCooldownMap.get(killer).contains(player)) {
                    for (String oppGod : Variables.opposingGods.get(god)) {
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        for (int i = e.getDrops().size() - 1; i >= 0; i--) {
            ...
            if (remove) {
                ...
                if (OHItems.isItemSimilarTo(item, OHItems.hermesElytra, false)) {
                    e.getDrops().add(GodData.hermesChestplateMap.get(player));
                    GodData.hermesChestplateMap.remove(player);
                } else if (OHItems.isItemSimilarTo(item, OHItems.poseidonBoots, false)) {
