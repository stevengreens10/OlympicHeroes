...
public class MultiBlockListener implements Listener {
                ...
                if (block.equals(mb.triggerBlock)) {
                    ...
                    if (prayed) {
                        if (!Cooldowns.prayCooldown.contains(e.getPlayer())) {
                            ...
                            for (String god : Variables.opposingGods.get(mb.god)) {
                            ...
                            if (canPray) {
                                ...
                                if (levelBefore != player.getLevel(mb.god)) {
                                    e.getPlayer().sendMessage("Your level for " + mb.god + " has just increased to "
                                            + player.getLevel(mb.god));
                                }

                                e.getItem().setAmount(e.getItem().getAmount() - 1);

                                Cooldowns.prayCooldown.add(e.getPlayer());
                                Cooldowns.removeCooldown(plugin, Cooldowns.prayCooldown, e.getPlayer(),
                                        Variables.PRAYER_COOLDOWN);
    ...
    public boolean isShrine(Block triggerBlock, Material exterior, byte exteriorData, Material stair, String god) {
        ...
        if (w.getBlockAt(l.clone().add(0, 1, 0)).getType() == exterior
                && w.getBlockAt(l.clone().add(0, -1, 0)).getType() == exterior) {
                ...
                for (int z = -1; z <= 1; z += 2) {
                    ...
                    if (w.getBlockAt(l.clone().add(0, -2, z)).getType() == stair
                        ...
                        if (((Stairs) w.getBlockAt(l.clone().add(0, -2, z)).getState().getData()).isInverted()
                                && ((Stairs) w.getBlockAt(l.clone().add(1, -1, z)).getState().getData())
                                        .getAscendingDirection() == BlockFace.EAST
                                && ((Stairs) w.getBlockAt(l.clone().add(-1, -1, z)).getState().getData())
                                        .getAscendingDirection() == BlockFace.WEST) {
                            // z = -1: north ; z = 1: south
                            if (z == -1 && ((Stairs) w.getBlockAt(l.clone().add(0, -2, z)).getState().getData())
                                    .getAscendingDirection() == BlockFace.SOUTH) {
                                facing = BlockFace.NORTH;
                                isShrine = true;
                            } else if (z == 1 && ((Stairs) w.getBlockAt(l.clone().add(0, -2, z)).getState().getData())
                                    .getAscendingDirection() == BlockFace.NORTH) {
                ...
                for (int x = -1; x <= 1; x += 2) {
                    ...
                    if (w.getBlockAt(l.clone().add(x, -2, 0)).getType() == stair
                        ...
                        if (((Stairs) w.getBlockAt(l.clone().add(x, -2, 0)).getState().getData()).isInverted()
                                && ((Stairs) w.getBlockAt(l.clone().add(x, -1, -1)).getState().getData())
                                        .getAscendingDirection() == BlockFace.NORTH
                                && ((Stairs) w.getBlockAt(l.clone().add(x, -1, 1)).getState().getData())
                                        .getAscendingDirection() == BlockFace.SOUTH) {
                            // z = -1: west ; z = 1: east
                            if (x == -1 && ((Stairs) w.getBlockAt(l.clone().add(x, -2, 0)).getState().getData())
                                    .getAscendingDirection() == BlockFace.EAST) {
                                facing = BlockFace.WEST;
                                isShrine = true;
                            } else if (x == 1 && ((Stairs) w.getBlockAt(l.clone().add(x, -2, 0)).getState().getData())
                                    .getAscendingDirection() == BlockFace.WEST) {
